...
if [ -f /run/shinage-system-uuid ]; then
    export SHINAGE_SCREEN_UUID="$(cat /run/shinage-system-uuid)"
fi
...
/usr/bin/chromium --noerrdialogs --incognito \
    --disable-translate \
    --disable-translate-new-ux \
    --disable-features=Translate,TelemetryExtension \
    --disk-cache-size=0 \
